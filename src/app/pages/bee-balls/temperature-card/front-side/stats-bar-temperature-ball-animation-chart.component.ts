...
@Component({
  selector: 'ngx-stats-bar-temperature-ball-animation-chart',
  template: `
    <div echarts
         [options]="options"
         class="echart"
...
export class StatsBarTemperatureBallAnimationChartComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    this.theme.getJsTheme()
      ...
      .subscribe(config => {
        const temperatureBarAnimationEchart: any = config.variables.profitBarAnimationEchart;

        this.setChartOption(temperatureBarAnimationEchart);
    });
  }
  ...
  setChartOption(chartVariables) {
    this.options = {
      ...
      legend: {
        data: ['temperature in', 'temperature out'],
        borderWidth: 0,
        borderRadius: 0,
        itemWidth: 15,
        itemHeight: 15,
